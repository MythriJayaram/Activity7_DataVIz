...
.range(d3.schemeTableau10);  // Color scheme from Tableau10
...
// Create the circles (scatter plot points)
...
function brushStart() {
// If no selection exists, we can reset styles (optional)
if (d3.brushSelection(this)[0][0] === d3.brushSelection(this)[1][0]) {
// Optional reset behavior when brushing starts
}
}
...
// Interactivity in the legend (toggle circles visibility)
legends_items.on("click", function(event, d) {
const isSelected = d3.select(this).classed("selected");
const circles = Fig.selectAll(`.${d}`);
